...
const BarItem: React.FC<BarItemProps> = ({ icon: Icon, label, href, currentPath, badgeCount }) => {
  ...
  return (
    <div
      ...
      className={`relative flex flex-col hover:bg-pink-600/20 pb-2 items-center p-1 px-3 gap-1 rounded-2xl ${isActive ? 'bg-pink-600/20' : 'bg-transparent'}`}
    >
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full border-t-2 drop-shadow-xl bg-pink-400/25 bg-white shadow-pink-500 border-pink-600 rounded-t-2xl">
      <div className="flex justify-center gap-x-2 items-center py-2">
        <BarItem icon={Ticket} label="My Coupons" href="/" currentPath={currentPath} />
        <BarItem icon={LifeBuoy} label="Support" href="/support" currentPath={currentPath} />
        <BarItem icon={TrendingDown} label="Issued Req" href="/issuedrequest" currentPath={currentPath} badgeCount={5} />
        <BarItem icon={Gift} label="Redeem Req" href="/redeemrequest" currentPath={currentPath} badgeCount={3} />
      </div>
    </div>
  );
};
